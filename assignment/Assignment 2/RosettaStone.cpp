...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    if (sum == 0.0) {
        error("map doesn't contain any nonzero number!");
    }

    sum = sqrt(sum);
    for (string key : input) {
        result[key] = input[key] / sum;
    }
    return result;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    Corpus maxSimiCor;
    double maxCosSimilarity = 0.0;
    for (Corpus cor : corpora) {
        double cosSimilarity = cosineSimilarityOf(textProfile, cor.profile);
