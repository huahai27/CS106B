...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> st(terrain.numRows(), terrain.numCols(), false);
    ...
    for (GridLocation source : sources) {
        ...
        while (!queue.isEmpty()) {
            GridLocation cell = queue.dequeue();
            st[cell] = true;
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (terrain.inBounds(x ,y) && st[x][y] == false && terrain[x][y] <= height) {
                    GridLocation underwater(x, y);
                    queue.enqueue(underwater);
                    st[x][y] = true;
                }
            }
        }
