...
int sumTheValue(const Set<Shift> chosen) {
    int sum = 0;
    for (Shift s : chosen) {
        sum += valueOf(s);
    }
...
bool valid_check(const Set<Shift>& chosen, const Shift& choose, const int maxHours) {
    // check overlap
    for (Shift c : chosen) {
        if (overlapsWith(c, choose)) {
            return false;
        }
    }

    // check time limit
    int chosen_length = lengthOf(choose);
    for (Shift c : chosen) {
        chosen_length += lengthOf(c);
    }
    if (chosen_length > maxHours) {
        return false;
    }
...
Set<Shift> highestValueScheduleForHelper(Set<Shift>& shifts, Set<Shift>& chosen, int maxHours) {
    // choice is empty
    if (shifts.isEmpty()) {
        return chosen;
    }
    ...
    Set<Shift> select = chosen + choose;
    Set<Shift> remaining = shifts - choose;
    if (!valid_check(chosen, choose, maxHours)) {
        return highestValueScheduleForHelper(remaining, chosen, maxHours);
    } else {
        Set<Shift> result1 = highestValueScheduleForHelper(remaining, chosen, maxHours);
        Set<Shift> result2 = highestValueScheduleForHelper(remaining, select, maxHours);

        // compare the value
        if (sumTheValue(result1) > sumTheValue(result2)) {
            return result1;
        } else {
            return result2;
        }
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    Set<Shift> chosen = {};
    if (maxHours < 0) {
        error("The maxHours must be greater than 0!");
    }
    ...
    return highestValueScheduleForHelper(copy, chosen, maxHours);
}
