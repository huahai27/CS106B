...
void allEmphasesOfHelper(Vector<string>& tokens, Set<string>& ans, string& chosen, int len, int idx) {
    if (idx == len) {
        ans.add(chosen);
    } else if (!isalpha(tokens[idx][0])) {
        chosen += tokens[idx];
        allEmphasesOfHelper(tokens, ans, chosen, len, idx + 1);
    } else {
        string backup = chosen;
        string upper = toUpperCase(tokens[idx]);
        string lower = toLowerCase(tokens[idx]);
        chosen += upper;
        allEmphasesOfHelper(tokens, ans, chosen, len, idx + 1);
        chosen = backup + lower;
        allEmphasesOfHelper(tokens, ans, chosen, len, idx + 1);
...
Set<string> allEmphasesOf(const string& sentence) {
    ...
    string chosen = "";
    int len = tokens.size();
    allEmphasesOfHelper(tokens, ans, chosen, len, 0);
    return ans;
}
