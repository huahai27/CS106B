 ...
 */
double weightOnBackOfHelper(int row, int col, int pyramidHeight, Grid<double>& table) {
    /* TODO: Delete the next few lines and implement this function. */
    if (row < 0 || col < 0) {
        error("The coordinate point must be greater than or equal to 0");
    }
    if (col > row || row + 1 > pyramidHeight) {
        error("The coordinates are out of bounds");
    }
    ...
    if (row == 0 && col == 0) {
        ...
        double weight = 0;
        if (col - 1 >= 0) {
            if (fabs(table[row - 1][col - 1] - 0.0) < 1e-6) {
                table[row - 1][col - 1] = weightOnBackOfHelper(row - 1, col - 1, pyramidHeight - 1, table);
            }
            weight += 80.0 + table[row - 1][col - 1] / 2;
        }
        if (col + 1 <= row) {
            if (fabs(table[row - 1][col] - 0.0) < 1e-6) {
                table[row - 1][col] = weightOnBackOfHelper(row - 1, col, pyramidHeight - 1, table);
            }
            weight += 80.0 + table[row - 1][col] / 2;
        }
...
double weightOnBackOf(int row, int col, int pyramidHeight) {
    Grid<double> table(pyramidHeight, pyramidHeight, 0.0);
