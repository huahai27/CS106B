...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    for (const auto& state : populations) {
        int p = populations[state];
        pq.enqueue(DataPoint(state, static_cast<double>(-p) / sqrt(2.0)));
        ret.put(state, 1);
    }

    while (numSeats > 0) {
        auto data = pq.dequeue();
        string state = data.name;
        ret[state]++;
        numSeats--;
        double p = populations[state] / sqrt(static_cast<double>(ret[state] * (ret[state] + 1)));
        pq.enqueue(DataPoint(state, -p));
    }
