...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if (freq.size() < 2) error("The input string must have at least two different characters in it");
    PriorityQueue<EncodingTreeNode*> pq;
    for (char c : freq) {
        EncodingTreeNode* node = new EncodingTreeNode({ c, nullptr, nullptr });
        pq.enqueue(node, freq[c]);
    }

    while (pq.size() != 1) {
        EncodingTreeNode* node = new EncodingTreeNode;
        int priZero = pq.peekPriority();
        EncodingTreeNode* nodeZero = pq.dequeue();
        int priOne = pq.peekPriority();
        EncodingTreeNode* nodeOne = pq.dequeue();

        node->zero = nodeZero;
        node->one = nodeOne;
        node->ch = ' ';
        pq.enqueue(node, priZero + priOne);
    }
...
string decodeText(Queue<Bit>& bits, EncodingTreeNode* tree) {
    ...
    while (!bits.isEmpty()) {
        Bit now = bits.dequeue();

        if (now == 0) curr = curr->zero;
        else curr = curr->one;
 ...
 */
void makeTable(Map<char, string>& table, EncodingTreeNode* tree, string path) {
    if (isLeaf(tree)) {
        table[tree->ch] = path;
    }
    else {
        makeTable(table, tree->zero, path + "0");
        makeTable(table, tree->one, path + "1");
    }
...
EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    Bit now = bits.dequeue();
    char ch;
    EncodingTreeNode* zeroNode;
    EncodingTreeNode* oneNode;

    if (now == 1) {
        ch = ' ';
        zeroNode = decodeTree(bits, leaves);
        oneNode = decodeTree(bits, leaves);
    } else {
        ch = leaves.dequeue();
        zeroNode = nullptr;
        oneNode = nullptr;
    ...
    EncodingTreeNode* node = new EncodingTreeNode({ ch, zeroNode, oneNode });
    return node;
}
...
void encodeTree(EncodingTreeNode* tree, Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    if (!isLeaf(curr)) {
        bits.enqueue(1);
        encodeTree(curr->zero, bits, leaves);
        encodeTree(curr->one, bits, leaves);
    } else {
        bits.enqueue(0);
        char c = curr->ch;
        leaves.enqueue(c);
...
HuffmanResult compress(const string& text) {
    ...
    HuffmanResult ret = { treeBits, treeLeaves, messageBits };
    deleteTree(tree);
...
string decompress(HuffmanResult& file) {
    /* TODO: Delete this comment and the next few lines, then implement this. */
    Queue<Bit> tb = file.treeBits;
    Queue<char> tl = file.treeLeaves;
    Queue<Bit> mb = file.messageBits;

    EncodingTreeNode* tree = decodeTree(tb, tl);
    string ret = decodeText(mb, tree);
    deleteTree(tree);
