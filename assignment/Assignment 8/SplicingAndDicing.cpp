...
Nucleotide* toStrand(const string& str) {
    ...
    for (size_t i = 0; i < len; ++i) {
        Nucleotide* dna = new Nucleotide{str[i], nullptr, prevNode};
 ...
 */
bool findFirstHelper(Nucleotide* dna, Nucleotide* target) {
    if (dna != nullptr) {
        Nucleotide* ptr1 = dna;
        Nucleotide* ptr2 = target;
        ...
        if (ptr1->value == ptr2->value) {
            ...
            while (ptr2 != nullptr) {
                if (ptr1 == nullptr || ptr1->value != ptr2->value) return false;
                ptr1 = ptr1->next;
                ptr2 = ptr2->next;
            }
            if (ptr2 == nullptr) {
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    Nucleotide* prev = head->prev;
    Nucleotide* end = head;
    Nucleotide* ptr = target;
    ...
    Nucleotide* lastToDelete = (end != nullptr) ? end->prev : nullptr;
    ...
    // Nucleotide* lastToDelete = head;
    // ptr = target;
    // while (ptr->next != nullptr) {
    //     lastToDelete = lastToDelete->next;
    //     ptr = ptr->next;
    // }
