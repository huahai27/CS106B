...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    int count = 0;
    while (elems[idx].type != SlotType::EMPTY) {
        if (elems[idx].type == SlotType::FILLED && elems[idx].value == elem) {
            return false;
        }
        else if (elems[idx].type == SlotType::TOMBSTONE && firstTombstone == -1) {
            firstTombstone = idx;
        }

        idx = (idx + 1) % allocatedSize;
        if (++count > allocatedSize) break;
    }

    idx = (firstTombstone == -1) ? idx : firstTombstone;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    while (true) {
        ...
        SlotType status = elems[i].type;
        if (status == SlotType::EMPTY) return false;
        else if (status == SlotType::TOMBSTONE) continue;
        else if (elems[i].value == elem) {
            idxIfRemove = i;
            return true;
        }
    }
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    elems[idxIfRemove].type = SlotType::TOMBSTONE;
    logicalSize--;
    return true;
}
