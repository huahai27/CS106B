...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    if (this->contains(elem) || this->size() == allocatedSize) return false;

    Slot node = { elem, 0 };
    int idx = HashFn(elem) % allocatedSize;

    while (elems[idx].distance != EMPTY_SLOT) {
        if (node.distance > elems[idx].distance) {
            swap(node, elems[idx]);
        }
        idx = (idx + 1) % allocatedSize;
        node.distance++;

        if (node.distance >= allocatedSize) {
            return false;
        }
    }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    while (dist != allocatedSize) {
        if (elems[idx].distance != EMPTY_SLOT && elems[idx].value == elem) {
            idxIfRemove = idx;
            return true;
        }
        else if (elems[idx].distance == EMPTY_SLOT || elems[idx].distance < dist) return false;
        else {
            idx = (idx + 1) % allocatedSize;
            dist++;
        }
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    elems[idxIfRemove].distance  = EMPTY_SLOT;
    int idx = (idxIfRemove + 1) % allocatedSize;
    logicalSize--;

    while (elems[idx].distance != 0 && elems[idx].distance != EMPTY_SLOT) {
        int prev = (idx - 1 + allocatedSize) % allocatedSize;
        swap(elems[idx], elems[prev]);
        elems[prev].distance--;
        idx = (idx + 1) % allocatedSize;
    }
