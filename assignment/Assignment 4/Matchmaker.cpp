...
bool alreadyMatched(const string& name, const Set<Pair>& matching) {
    for (const Pair& p : matching) {
        if (name == p.first() || name == p.second()) return true;
    }
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    if (possibleLinks.isEmpty()) return true;
        ...
        Set<string> ps = possibleLinks.get(p1);

        for (const string& p2 : ps) {
            if (!alreadyMatched(p2, matching)) {
                Pair newPair(p1, p2);

                // dfs
                matching.add(newPair);
                Map<string, Set<string>> left = possibleLinks;
                left.remove(p1), left.remove(p2);
                if (hasPerfectMatching(left, matching)) return true;
...
Set<Pair> maximumWeightMatchingHelper(const Map<string, Map<string, int>>& possibleLinks, int& maxWeight) {
    Set<Pair> pairs;
    if (possibleLinks.isEmpty()) return pairs;
    else {
        string p1 = possibleLinks.firstKey();

        // not choose p1
        int maxWithOut_p1 = maxWeight;
        Map<string, Map<string, int>> psWithOut_p1 = possibleLinks;
        psWithOut_p1.remove(p1);
        Set<Pair> PairWithOut_p1 = maximumWeightMatchingHelper(psWithOut_p1, maxWithOut_p1);

        // choose p1
        int maxWith_p1 = maxWeight;
        Set<Pair> PairWith_p1;

        Map<string, int> ps1 = possibleLinks.get(p1);
        for (string p2 : ps1) {
            // !!! Do not perform duplicate searches
            if (!possibleLinks.containsKey(p2)) continue;

            int weight = ps1.get(p2);
            if (weight <= 0) continue;

            Pair newpair(p1, p2);
            Map<string, Map<string, int>> left = possibleLinks;
            left.remove(p1), left.remove(p2);
            int currentMax = maxWeight + weight;
            Set<Pair> currentPair = maximumWeightMatchingHelper(left, currentMax);
            currentPair.add(newpair);

            // update
            if (currentMax > maxWith_p1) {
                maxWith_p1 = currentMax;
                PairWith_p1 = currentPair;
            }
        }

        if (maxWith_p1 > maxWithOut_p1) {
            maxWeight = maxWith_p1;
            return PairWith_p1;
        } else {
            maxWeight = maxWithOut_p1;
            return PairWithOut_p1;
        }
    }
}
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    ...
    Set<Pair> ans, pairs;
    int maxWeight = 0;
    ans = maximumWeightMatchingHelper(possibleLinks, maxWeight);
