...
bool canBeMadeDisasterReadyHelper(const Map<string, Set<string>>& roadNetwork,
                                  ...
                                  Set<string>& Cities) {
    if (numCities == 0 && !Cities.isEmpty()) return false;
    else if (Cities.isEmpty() && numCities >= 0) return true;
    else {
        string city1 = Cities.first();
        Set<string> adj_city1 = roadNetwork.get(city1);

        // stockpile in that city itself
        Set<string> stkHere = Cities;
        stkHere.remove(city1);

        for (string c : adj_city1) {
            if (!Cities.contains(c)) continue;
            stkHere.remove(c);
        }

        supplyLocations.add(city1);
        --numCities;
        if (canBeMadeDisasterReadyHelper(roadNetwork, numCities, supplyLocations, stkHere)) return true;
        ++numCities;
        supplyLocations.remove(city1);

        //  stockpile in a neighboring city
        for (string city2 : adj_city1) {
            Set<string> adj_city2 = roadNetwork.get(city2);
            Set<string> stkAdj = Cities;
            stkAdj.remove(city2);

            for (string c : adj_city2) {
                stkAdj.remove(c);
            }

            supplyLocations.add(city2);
            --numCities;
            if (canBeMadeDisasterReadyHelper(roadNetwork, numCities, supplyLocations, stkAdj)) return true;
            ++numCities;
            supplyLocations.remove(city2);
        }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
                            int numCities,
                            Set<string>& supplyLocations) {
    if (numCities < 0) error("The numCities can't be negative!");
    ...
    return canBeMadeDisasterReadyHelper(roadNetwork, numCities, supplyLocations, Cities);
}
