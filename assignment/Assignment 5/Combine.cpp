...
Vector<DataPoint> merge_sort(const Vector<DataPoint>& seq1,
    ...
    while (p1 < len1 && p2 < len2) {
        if (seq1[p1].weight < seq2[p2].weight) {
            ans.add(seq1[p1++]);
        }
        else if (seq1[p1].weight > seq2[p2].weight){
            ans.add(seq2[p2++]);
        }
        else {
            ans.add(seq1[p1++]);
    ...
    if (p1 < len1) {
        for (int i = p1; i < len1; ++i) ans.add(seq1[i]);
    }
    if (p2 < len2) {
        for (int i = p2; i < len2; ++i) ans.add(seq2[i]);
    }
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    if (sz == 0) return ans;
    if (sz == 1) return sequences[0];
    else if (sz == 2) return merge_sort(sequences[0], sequences[1]);
    else {
        int mid = sz / 2;
        auto seq1 = combine(sequences.subList(0, mid));
        auto seq2 = combine(sequences.subList(mid));

        return merge_sort(seq1, seq2);
    }
}
