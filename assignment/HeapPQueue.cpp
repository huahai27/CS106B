...
HeapPQueue::HeapPQueue() {
    ...
    elems = new DataPoint[allocatedSize + 1];
}
...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    if (logicalSize == allocatedSize - 1) {
        ...
        DataPoint* tmp = new DataPoint[allocatedSize + 1];
    ...
    elems[idx] = data;
    int par_idx = idx / 2;
    while (par_idx >= 1 && elems[idx].weight < elems[par_idx].weight) {
        swap(elems[idx], elems[par_idx]);
        idx = par_idx;
        par_idx = idx / 2;
    }
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    DataPoint ret = elems[logicalSize--];

    int idx = 1;
    int chi_idx = 2 * idx;
    if (chi_idx + 1 <= logicalSize) {
        chi_idx = (elems[idx * 2].weight < elems[idx * 2 + 1].weight) ? idx * 2 : idx * 2 + 1;
    }

    while (chi_idx <= logicalSize && elems[idx].weight > elems[chi_idx].weight) {
        swap(elems[idx], elems[chi_idx]);
        idx = chi_idx;
        chi_idx = 2 * idx;
        if (chi_idx + 1 <= logicalSize) {
            chi_idx = (elems[idx * 2].weight < elems[idx * 2 + 1].weight) ? idx * 2 : idx * 2 + 1;
